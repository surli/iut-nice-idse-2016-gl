...
public class GameRest {
	...
	/**
	 * Retourne si la partie a commencée gamename : Nom de la partie
	 * 
	...
	public Response isStarted(@PathParam("gamename") String gamename) {
		...
		return Response.status(200)
				.entity("{\"state\": " + game.getBoard().gameBegin() + "}")
				.build();
	}

	/**
	 * Méthode en POST permettant l'ajout d'un joueur dans une partie Signature
	 * : {_token: token, pseudo: String} La partie doit être existante. Renvoie
	 * {status: boolean}
	 * 
	 * @return Response
	 */
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response addPlayer(@PathParam("gamename") String gamename,
			String objJSON) throws JSONException {
		...
		if (!json.has("pseudo"))
			return Response.status(405).entity("Missing or invalid parameters")
					.build();
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response beginGame(@PathParam("gamename") String gamename,
			String objJSON) throws JSONException {
		// Initialisation des objets
		Model model = Model.getInstance();
		Game game = model.findGameByName(gamename);
		...
		if (model.findGameByName(gamename).getNumberPlayers() == model
				.findGameByName(gamename).getBoard().getPlayers().size())
			if (model.startGame(gamename, json.getString("pseudo")))
				return Response.status(200).entity("{\"status\": true}")
						.build();
	...
	/**
	 * Méthode en GET permettant de recuperer le joueur devant jouer La partie
	 * doit être existante. Renvoie {"pseudo": String}
	 * 
	 * @return Response
	 */
	@GET
	...
	public Response actualPlayer(@PathParam("gamename") String gamename) {
		...
		if (!game.gameBegin()) {
			return Response.status(401)
					.entity("{\"error\":\"Game has not begin\"}").build();
		}
		...
		if (currentPlayer == null) {
			return Response.status(422)
					.entity("{\"error\":\"No current player has been set\"}")
					.build();
		}

		return Response.status(200)
				.entity("{\"pseudo\":\"" + currentPlayer.getName() + "\"}")
				.build();
	}
	...
	@GET
	@Path("{gameName}/{pseudo}/hand")
	@Produces(MediaType.APPLICATION_JSON)
	public Response handplayer(@PathParam("gameName") String gameName, @PathParam("pseudo") String playerName) throws JSONException {
		Model model = Model.getInstance();
